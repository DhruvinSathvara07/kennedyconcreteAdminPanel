...
const ShotcreateList = () => {
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this item?')) {
      try {
        await axios.delete(`http://localhost:5000/shotcreate/${id}`)
    ...
    <div>
      <div className="my-5">
        <div className="d-flex justify-content-between align-items-center mb-4">
          <h1>ShotCreate List</h1>
          <button
            className="btn btn-success"
            onClick={() => navigate('/theme/concrete/page')} // Adjust path as needed
          >
            Add New Item
          </button>
        </div>
                ...
                {showData.map((item, index) => {
                  ...
                  return (
                    <tr key={item._id || index}>
                      ...
                      {/* <td>
                        <div style={{ maxWidth: '200px' }}>{item.para}</div>
                      </td> */}
                      <td>
                        {item.video_link ? (
                          <a
                      ...
                      <td>
                        <div className="d-flex gap-2">
                          <i
                            ...
                            onClick={() => navigate(`/theme/shotcrete/list/${item._id}`)}
                            title="Edit"
                          ></i>
                          <i
