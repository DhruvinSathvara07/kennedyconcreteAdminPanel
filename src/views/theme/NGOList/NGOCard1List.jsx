...
const NGOCard1List = () => {
  ...
  const showServicesData = () => {
    console.log('hello')
    axios.get('http://localhost:3001/PartnerCard').then((response) => {
  ...
  const handleDelete = async (id) => {
     if (confirm("Successfully Deleted")) {
      await axios.delete(`http://localhost:3001/PartnerCard/${id}`)
      .then((response) => {
                ...
                <tr key={index}>
                  ...
                  <i className="fa-solid fa-trash me-5 ps-5 cursor" onClick={() => handleDelete(items.id)}></i>
                    <i
                      ...
                      onClick={() => navigate(`/EditPages/Ngo/EditNgoCard1List/${items.id}`)}
                    ></i>
                  </td>
                </tr>
              )
